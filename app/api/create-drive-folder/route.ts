...
export async function POST(request: Request) {
  ...
  const folderName = firstName
    ? `Job Pipeline Resumes — ${firstName}`
    : 'Job Pipeline Resumes'

  const env = { ...process.env, PATH: `/usr/local/bin:${process.env.PATH ?? ''}` }

  try {
    let raw: string
    const cmd = `gws drive files create --json '${JSON.stringify({ name: folderName, mimeType: 'application/vnd.google-apps.folder' })}'`
    try {
    ...
    const webViewLink = (result.webViewLink as string | undefined) ?? `https://drive.google.com/drive/folders/${id}`

    return Response.json({ success: true, id, name: folderName, webViewLink })
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    return Response.json({ error: `Failed to create Drive folder: ${msg}` }, { status: 500 })
  }
}
