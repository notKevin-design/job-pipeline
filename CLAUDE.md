...
| v0.4 | JD-specific gap questions per job after JD fetch but before scoring (batched 4/widget, derived from JD vs. resume comparison); one-time globals widget for portfolio URL, location filter, and hard-skip industries; stale job gate skips postings >12 months old via AskUserQuestion; per-gap question loop in analyze-resume suppressed when gap_context already in PIPELINE CONTEXT (fallback preserved for standalone use) | Local (Claude Code) |
